    ...
    <header className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-400", scrolled ? "bg-dark/97 border-b border-gold/10 backdrop-blur-md" : "bg-gradient-to-b from-dark/95 to-transparent")}>
      <div className="max-w-[1200px] mx-auto px-6 lg:px-12 h-[72px] flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <a key={item.label ?? item.key} href={item.href}
              className="relative text-[11px] tracking-[0.15em] uppercase text-cream/60 hover:text-gold-light transition-colors duration-200 group">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="md:hidden text-cream/70 hover:text-cream transition-colors p-1"
      ...
      {mobileOpen && (
        <div className="md:hidden bg-dark-2 border-t border-gold/10 px-6 py-6">
          <nav className="flex flex-col gap-5 mb-6">
            {navItems.map((item) => (
              <a key={item.label ?? item.key} href={item.href}
                className="text-[11px] tracking-[0.15em] uppercase text-cream/60 hover:text-gold-light transition-colors"
